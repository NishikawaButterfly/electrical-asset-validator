...
### "The XLSX workbook has 5 formulas with no stored result, the first in cell G2."

The workbook holds formulas that have never been calculated, so there is no
result to read. A formula is read as the result the spreadsheet stored for it,
and this file has none.

This means the register was **generated by a script** — openpyxl, pandas, a
reporting tool — and handed over without ever being opened. A file a person
saved from Excel or LibreOffice never produces this message.

Open it in Excel or LibreOffice and save it; that alone stores the results.
Replacing the formulas with values works too. The message names one cell so you
can find it in a file where only some formulas are affected, and reads
`a formula in cell H4 with no stored result` when there is just one. Nothing
was stored from the refused upload — correct the file and upload it again.
...
Check for unit suffixes (`400 V`), comma decimal separators (`18,5`), and text
placeholders (`TBC`). On an XLSX register, check whether the formulas behind the
ratings are broken: a formula is read as the result the workbook stored, so a
stored `#DIV/0!` or `#REF!` arrives as an invalid number. Fix it in the source
workbook and save it again.

Working formulas do not cause this. `=200*2` is read as `400`.
