...
2. **Formulas are fine — but save the file from a spreadsheet program.** A cell
   holding `=Design!G14` is read as the `400` your spreadsheet worked out, so
   there is no need to paste-special as values. What matters is that Excel or
   LibreOffice has saved the file at least once, because that is what stores
   the results. A register generated by a script and handed straight over
   contains no results at all and is refused:

   ```
   The XLSX workbook has 5 formulas with no stored result, the first in cell G2. ...
   ```

   Opening it and saving it fixes that; chapter [2](02-input-files.md) gives the
   message in full. Paste-special as values still works and is worth doing where
   the register is a deliverable in its own right — see the last section of this
   chapter.
...
- **Macros.** A register is evidence; it should not execute. Nothing in a
  workbook's macros is read, but a file that needs them is a file nobody can
  check by eye.
- **Formulas, in the final issued revision.** They are read correctly now, so
  this is no longer about the software. It is that a handover document whose
  numbers depend on a sheet nobody kept is not a record of anything. Work with
  formulas, issue values.
